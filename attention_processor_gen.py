...
class FluxAttnProcessor2_0:
    ...
    def __call__(
        ...
        attention_mask: Optional[torch.FloatTensor] = None,
        image_rotary_emb: Optional[torch.Tensor] = None,
        image_rotary_emb2: Optional[torch.Tensor] = None,
        ref_key: Optional[torch.Tensor] = None,
        ...
        if encoder_hidden_states is not None:
            ...
            key = torch.cat([key, ref_key[:,:,-(image_rotary_emb2[0].shape[0]-image_rotary_emb[0].shape[0]):]], dim=2)
            value = torch.cat([value, ref_value[:,:,-(image_rotary_emb2[0].shape[0]-image_rotary_emb[0].shape[0]):]], dim=2)
        
        if image_rotary_emb is not None:
            from diffusers.models.embeddings import apply_rotary_emb
            query = apply_rotary_emb(query, image_rotary_emb)
            key = apply_rotary_emb(key, image_rotary_emb2)         
            #key = apply_rotary_emb(key, image_rotary_emb)
        
        # 1, 24, TOKENS, HIIDEN DIM
        #key = torch.cat([key, ref_key], dim=2)
        #value = torch.cat([value, ref_value], dim=2)
