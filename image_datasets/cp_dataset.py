...
class VitonHDDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        default_cloth_prompt = "A detailed photo of a garment."
        default_model_prompt = "A women against a white background."

        # Prompt
        if os.path.exists(os.path.join(self.dataroot, self.phase, "cloth", c_name + ".txt")):
            cloth_prompt = open(os.path.join(self.dataroot, self.phase, "cloth", c_name + ".txt")).read()
        else:
            cloth_prompt = default_cloth_prompt

        if os.path.exists(os.path.join(self.dataroot, self.phase, "image", im_name + ".txt")):
            image_prompt = open(os.path.join(self.dataroot, self.phase, "image", im_name + ".txt")).read()
        else:
            image_prompt = default_model_prompt
        ...
        result["cloth_prompt"] = cloth_prompt
...
if __name__ == "__main__":
    dataset = VitonHDDataset("/workspace1/pdawson/tryon-scraping/vitonhd", "train", 
